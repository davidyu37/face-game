function MouthDetection(cb) {
  let faceapi;
  let video;
  let detections;
  let width = 360;
  let height = 280;
  let canvas, ctx, message;
  let brain = null;
  let state = "closed";
  ...
  async function make() {
    ...
    message.innerText = "Webcam video loading...";
  ...
  function gotResult(err, result) {
    ...
    if (result) {
      ...
      // Only Trigger when state switch
      if (normal.confidence < open.confidence) {
        message.innerText = "Mouth Opened";
        if (state !== "open") {
          state = "open";
          cb("jump");
        }
      ...
      message.innerText = "Mouth Not Opened";
      state = "closed";
      cb("dont-jump");
    }
  }
